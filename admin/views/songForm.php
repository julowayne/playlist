...
<h3>Formulaire des chansons</h3>
	...
	<form action="index.php?controller=songs&action=<?= isset($song) || (isset($_SESSION['old_inputs']) && $_GET['action'] == 'edit') ? 'edit&id='.$_GET['id'] : 'add' ?>" method="post" enctype="multipart/form-data">
		...
		<option value="<?= $album['id'] ?>" <?php if (isset($song) && $song['album_id'] == $album['id']): ?> selected="selected" <?php endif;?>><?= isset($album) ? $album['name'] : '' ?></option>
		<?php endforeach;?>
		</select><br>

		<label for="title">Titre :</label>
		<input  type="text" name="title" id="title" value="<?= isset($song) ? $song['title'] : '' ?><?= isset($_SESSION['old_inputs']) ? $_SESSION['old_inputs']['title'] : '' ?>" /><br>
